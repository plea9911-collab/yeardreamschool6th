...
/* -------------------------------------------------------------------
   ...
   ------------------------------------------------------------------- */
SELECT track_name, media_type FROM tracks INNER JOIN media_types ON tracks.MediaTypeId = media_types.MediaTypeId ;
...
SELECT p.name,
        t.name
    FROM playlists p
    INNER JOIN playlist_track pn ON p.playlistId = pn.playlistId
    INNER JOIN tracks t ON pn.TrackId = t.TrackId
;
...
-- 메인
SELECT BillingCountry, Total_sal
    FROM (SELECT BillingCountry, SUM(Total) AS Total_sal
            FROM invoices
            GROUP BY BillingCountry) AS country_sal
