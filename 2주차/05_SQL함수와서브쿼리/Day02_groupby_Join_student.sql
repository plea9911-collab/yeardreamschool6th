...
SELECT ArtistID, COUNT(*) album_count
    FROM albums
    GROUP BY ArtistId
    ;
...
SELECT AlbumId, COUNT(*) AS track_cut
    FROM tracks
    GROUP BY AlbumId
    HAVING COUNT(*) > 15
    ;
...
SELECT BillingCity, SUM(Total) AS
    FROM invoices
...
SELECT CustomerId, COUNT(*) AS invoice_cut
    FROM invoices
    GROUP BY CustomerId
    HAVING COUNT(*) >= 7
    ;
...
-- 트랙이 많은 순으로 보여주세요.
SELECT GenreId, COUNT(*) AS track_cut
    FROM tracks
    GROUP BY GenreId
    HAVING COUNT(*) >= 100
    ORDER BY 2 DESC
