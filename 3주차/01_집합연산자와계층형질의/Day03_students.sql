...
SELECT GROUP_CONCAT(NAME ORDER BY NAME DESC) AS 트랙목록
FROM tracks
WHERE AlbumId = 1
;
...
-- (b) 구매 이력 유무별 고객 분류
-- customers, invoices
-- 출력 : customerId, 고객명(firstname, lastname) as 고객명, 
                          --  'Has Invoice' / 'No Invoice'
-- 힌트 : where 절 customers ID 활용해서 서브쿼리
...
-- 고객 이름 궁금함. 고객ID, FristName, LastName
...
WITH RECURSIVE emp_hierarchy(
    Employee, FristName, LastName, Title, ReportsTo, lvl
) AS (
    -- Achor : 최상의 직원
    SELECT EmployeeId, FristName, LastName, Title, ReportsTo, 0 AS lvl
    FROM employees
...
SELECT 
    lvl AS 계층레벨
    , EmployeeId AS 직원아이디
    , SUBSTR('           ', 1, lvl * 4) FristName || ' ' || LastName as 직원명
    , Title AS 
    , ReportsTo
FROM emp_hierarchy
