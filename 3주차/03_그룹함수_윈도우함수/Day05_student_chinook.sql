...
SELECT YearMonth
        ...
        , LAG(SUM(Total), 1) OVER (ORDER BY strftime('%Y-%m'), InvoiceDate) AS PrevMonthTotal
        , ROUND(SUM(Total), 1) - LAG(SUM(Total), 1) OVER (ORDER BY strftime('%Y-%m'), InvoiceDate) AS MoM_Diff
FROM invoices
ORDER BY strftime('%Y-%m') InvoiceData
ORDER BY YearMonth
;
...
SELECT EmployeeId, 
        ...
        Title,
        HireDate,
        NTILE(3) OVER (ORDER BY MAX(HireDate)) AS HireGroup
FROM employees
ORDER BY HireDate
;
...
SELECT g.Name AS Genre,
        ...
        , COUNT(t.TrackId) AS TrackCount -- 장르와 미디어타입별로 트랙의 수(TrackCount)
        , ROUND(AVG(t.UnitPrice)) AS AvgPrice -- 장르와 미디어타입별로 평균 단가(AvgPrice)
FROM tracks t
